...
local function perfectAim(targetPart)
    ...
    local predictionOffset = targetVelocity * Settings.Aimbot.PredictionMultiplier
    local finalPosition = targetPosition + predictionOffset
    
    local screenPosition = camera:WorldToViewportPoint(finalPosition)
    local mousePosition = UserInputService:GetMouseLocation()
    
    mousemoverel(
        (screenPosition.X - mousePosition.X) * Settings.Aimbot.Smoothness,
        (screenPosition.Y - mousePosition.Y) * Settings.Aimbot.Smoothness
    )
end
...
local function GetClosestPlayerToMouse()
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= localPlayer and 
           ...
           player.Character:FindFirstChild(AimSettings.TargetPart) and
           (not Settings.Aimbot.TeamCheck or not isTeammate(player)) then
